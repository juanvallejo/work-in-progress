...
public final class NPCSpawns {
	...
	public static final void packNPCSpawns() {
		...
		File packedSpawnsDir = new File("data/npcs/packedSpawns");
		
		// remove existing packed spawns before continuing
		if (packedSpawnsDir.exists()) {
			deleteDir(packedSpawnsDir);
		}
		if (!packedSpawnsDir.mkdir()) {
		...
		try {
			...
			Logger.log("NPCSpawns", "Finished packing npc spawns...");
			in.close();
		} catch (Throwable e) {
			Logger.handle(e);
		}
