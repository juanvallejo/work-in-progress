...
public class CrawlingHand extends NPC {

	private int stompId;

	public CrawlingHand(int id, WorldTile tile, int mapAreaNameHash,
	...
	public static void handleStomping(final Player player, final NPC npc) {
		...
		case 1648:
			if (player.getSkills().getLevel(18) < 1) {
				player.getPackets()
						.sendGameMessage(
								"You need at least a slayer level of 1 to fight this.");
				return;
			}
			break;
		default:
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.getInventory().addItem(8951, 1);
				stop();
			}
