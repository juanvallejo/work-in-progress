	...
	public int processWithDelay(Player player) {
		if (!isSuccesfull(player)) {
			...
			for (int i = 0; i <= index; i++) {
				...
				/**
				 * If Item id == 995, add coins to pouch.
				 */
				if (item.getId() == 995 && player.money + item.getAmount() > 0) {
					//player.getPackets().sendRunScript(5561, 1, item.getAmount());
					//player.money += item.getAmount();
					//player.refreshMoneyPouch();
					//player.out(player.getFormattedNumber(item.getAmount())+" coins have been added to your money pouch.");
				} else {
