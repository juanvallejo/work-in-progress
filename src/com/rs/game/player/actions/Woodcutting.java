...
private static Player player;
	...
	public static enum TreeDefinitions {
		...
		YEW_TREE(60, 200, 1515, 15, 17, 1341, 999999999, 9999),
		
		MAGIC_TREE(75, 260, 1513, 25, 21, 37824, 999999999, 99999),
	...
	private void addLog(Player player) {
		...
		if (definitions == TreeDefinitions.IVY) {
			...
			// todo gfx
		} else if (definitions == TreeDefinitions.YEW_TREE) {
				player.increaseFarmWc++;
				player.out("You get some yew logs from your tree.");
				if (player.increaseFarmWc >= Utils.getRandom(48)) {
					player.stopAll();
					player.getPackets().sendConfigByFile(701, 3);
					player.out("Your yew tree collapses as you have harvested all it's wood.");
					player.increaseFarmWc = 0;
					player.hasPlantedTree = false;
					player.yew = false;
				}
		} else if (definitions == TreeDefinitions.MAGIC_TREE) {
			player.increaseFarmWc++;
			player.out("You get some magic logs from your tree.");
			if (player.increaseFarmWc >= 25) {
				player.stopAll();
				player.getPackets().sendConfigByFile(701, 3);
				player.out("Your magic tree collapses as you have harvested all it's wood.");
				player.increaseFarmWc = 0;
				player.hasPlantedTree = false;
				player.magic = false;
			}
