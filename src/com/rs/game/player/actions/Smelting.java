...
public class Smelting extends Action {
	...
	public enum SmeltingBar {
		...
		CANNON_BALL(35, 25.56, new Item[] { new Item(2353) }, new Item(2, 4), -1),
	...
	public boolean start(Player player) {
		...
		if (bar == SmeltingBar.CANNON_BALL) {
	...
	public int processWithDelay(Player player) {
		...
		if (isSuccessFull(player)) {
			 if (bar == SmeltingBar.CANNON_BALL) {
					...
					player.sm("You heat the steel bar into a liquid state.");
					player.sm("You pour the molten metal into your cannonball mould.");
					player.sm("The molten metal cools slowly to form 4 cannonballs.");
					player.sm("You remove the cannonballs from the mould.");
			 } else {
			player.getInventory().addItem(bar.getProducedBar());
			player.getPackets().sendGameMessage(
