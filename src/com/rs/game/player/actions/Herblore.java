...
public class Herblore extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if(otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK || node.getId() == VIAL || node.getId() == COCONUT_MILK) {
			...
			// handle if player clicked vial first, or herb first
			String selectedItem = otherItem.getDefinitions().getName().toLowerCase();
			String actedUponItem = node.getDefinitions().getName().toLowerCase();

			player.getPackets().sendGameMessage("You add the " + selectedItem + " into the " + actedUponItem + ".");
	...
	public static boolean isIngredient(Item item) {
		...
		Ingredients ingredient = Ingredients.forId(item.getId());
