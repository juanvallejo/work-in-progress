...
public class PlayerCombat extends Action {
	...
	private boolean forceCheckClipAsRange(Entity target) {
		return target instanceof NexMinion
				...
				|| target instanceof SeaTrollQueen
	...
	public int processWithDelay(Player player) {
		...
		if (!player.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target)))
			...
			if (target instanceof SeaTrollQueen) { //Fixed SeaTrollQueen's Safe Spotting
				maxDistance = 0;
			}
	...
	private int getRangeCombatDelay(int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName.contains("shortbow")
					|| weaponName.contains("sagittarian shortbow")
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 18332: // Magic longbow (sighted)
				...
				//player.setNextGraphics(new Graphics(249, 0, 100));//TODO
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
					...
					dropAmmo(player, -1);
				if (weaponName.contains("dragon dart")) {
                        if (target instanceof Player) {
                            Player p2 = (Player) target;
                            p2.stopAll();
                        } else {
					...
					if (ammoId != -1 && Utils.getRandom(10) == 5) {
						...
						//	case 21364://TODO
							/*damage = getRandomMaxHit(player, weaponId,
									attackStyle, true, false, 1.15, true);
							target.setNextGraphics(new Graphics(466));
							soundId = 2917;
							break;*/
						default:
							damage = getRandomMaxHit(player, weaponId,
					...
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit * 3));
	...
	public int getArrowThrowGfxId(int weaponId, int arrowId) {
		if (arrowId == 884) {
			...
			return 22;
		} else if (weaponId == 21364) {
			return 466;
		} else if (weaponId == 10034) {
			return 909;
		} else if (weaponId == 10148) {
			return 438;//TODO
			//BOLAS gfx 467 - 469
		} else if (arrowId == 892)
			return 24;
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 23680:
				...
				if (target instanceof Player) {
					...
					if(damage > 0) {
						...
						// drain npc's defence when bgs special attack is used
						NPC targetNpc = (NPC) this.target;
						targetNpc.drainDefence(player, damage / 10, weaponId, attackStyle); ////--
			...
			case 23695:
				...
				if (hit > 0) {
					...
					hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 2.0, true);
					if (hit > 0) {
			...
			case 1434: // dragon mace
				...
				break;
			case 13468:
				if (player.getUsername().equalsIgnoreCase("brandonxxxxxxxxxxxxxxxxxxxxxxx")) {
				player.setNextAnimation(new Animation(1062));
				player.setNextGraphics(new Graphics(252, 0, 100));
			      delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 2000, true)),
							getMeleeHit(
									player,
									getRandomMaxHit(player, weaponId, attackStyle,
											false, true, 2000, true)),
											getMeleeHit(
													player,
													getRandomMaxHit(player, weaponId, attackStyle,
															false, true, 2000, true)),
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			...
			// npc damage modifier
				...
				def -= n.getDrainedDefenceModifier();
				def *= 2;

				// player.getPackets().sendGameMessage("Current NPC defence = " + def); ////--
...
/*			if (fullGuthansEquipped(player)) {
	...
	public void delayMagicHit(int delay, final Hit... hits) {
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("bow"))
					return 426;
				if (weaponName.contains("chinchompa")) {//TODO
					return 2779;
				}
