...
public class RoomReference implements Serializable {
	...
	public Room getCurrentRoom(Player player) {
		/*
		 * for (RoomReference room : player.getRooms()) { //room = new
		 * RoomReference(); if (room.getX() == player.getRoomX() && room.getY()
		 * == player.getRoomY() && room.getPlane() == player.getPlane()) {
		 * System.out.println(room.getRoom().getName()); return room.getRoom();
		 * } } return null;
		 */
		return null;
	}
