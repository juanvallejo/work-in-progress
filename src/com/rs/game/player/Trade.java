...
public class Trade {
	...
	public void removeItem(final int slot, int amount) {
		synchronized (this) {
			...
			synchronized (target.getTrade()) {
				...
				if (player.addedViaPouch == true && item.getId() == 995) {
					items.remove(slot, item);
					player.money += maxAmount;
					player.getPackets().sendRunScript(5561, 1, maxAmount);
	...
	public void addFromPouch(int value) {
		...
		synchronized (this) {
			synchronized (target.getTrade()) {
				...
				if(CloseTradeStage.DONE != stage) {
					...
					Player.tradeLog(player, message);
				}
