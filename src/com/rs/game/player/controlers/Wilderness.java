...
public class Wilderness extends Controler {
	...
	@Override
	public boolean processWildyTeleport(WorldTile toTile) {
		return true;
	}
	...
	public void pksystem() {
	if (pkkill >= 2) {
		player.inventory.addItem(12852, 5);
		return;
	}
	if (player.pkkill >= 10) {
			World.sendWorldMessage("Name is on a 10 kill streak!", false);
		return;
		}
	}*/
			...
			public void run() {
				if (loop == 0) {
					...
					if (killer != null) {
						...
						//killer.increasePkCount(player);
						//killer.inventory.addItem(12852, 1);
						//killer.pkkill += 1;
						//killer.getPackets().sendGameMessage("You recieve a pk point!");
						//killer.pksystem();
					}
	...
	public static final boolean isAtWild(WorldTile tile) {//TODO fix this
		return (tile.getX() >= 3011 && tile.getX() <= 3132
				...
				|| (tile.getX() >= 3000 && tile.getX() <= 3300
						&& tile.getY() >= 10000 && tile.getY() <= 10600);
	}
