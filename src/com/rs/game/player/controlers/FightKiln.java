...
public class FightKiln extends Controler {
	...
	public static void enterFightKiln(Player player, boolean quickEnter) {
		...
		if(!player.isCompletedFightCaves()) {
		    player.getPackets().sendGameMessage("Complete fightcaves before you try entering again.");
			return;
		}
		...
		if(type == 0) {
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					if (players == null)
						continue;
						players.getPackets().sendGameMessage("<img=6><col=ff8c38>News: "+player.getDisplayName()+" Just completed Fightkiln.");
					}
