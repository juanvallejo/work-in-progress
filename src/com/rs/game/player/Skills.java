...
public final class Skills implements Serializable {
	...
	public void addXp(int skill, double exp) {
		...
		if (player.isXpLocked())
			return;
		if (skill != PRAYER && skill != SLAYER && skill != CONSTRUCTION && skill != FARMING && skill != SUMMONING)
			exp *= player.isExtremeDonator() ? Settings.XP_RATE + 100 : player.isDonator() ? Settings.XP_RATE + 1 : Settings.XP_RATE;
