...
public class DuelArena extends Controler {
	...
	public void endDuel(final Player victor, final Player loser,
		...
		if (!tie) {
			...
			tie = false;
			victor.out("It was a draw!");
			//for (Item item : player.getLastDuelRules().getStake().getItems()) {
			//	if (item == null) continue;
			//	victor.getInventory().deleteItem(item);
			//	}
			//for (Item item : target.getLastDuelRules().getStake().getItems()) {
			//	if (item == null) continue;
			//	loser.getInventory().deleteItem(item);
			//	}
		}
	...
	private void giveItems() {
		...
		if (!tie) {
			for (Item item : winner.getLastDuelRules().getStake().getItems()) {
				if (item == null) {
					continue;
				}
				if (winner.getInventory().getFreeSlots() < 1) {
					World.addGroundItem(item, new WorldTile(winner.getX(),
							winner.getY(), winner.getPlane()), winner, false,
							600, false);
					winner.out("Your stake was dropped on the ground");
					dropped = true;
				} else {
					winner.getInventory().addItem(item);
				}
				for (Item item2 : loser.getLastDuelRules().getStake()
						.getItems()) {
					if (item2 == null) {
						continue;
					}
					if (winner.getInventory().getFreeSlots() < 1) {
						World.addGroundItem(item2, new WorldTile(winner.getX(),
								winner.getY(), winner.getPlane()), winner,
								false, 600, false);
						if (!dropped)
							winner.out("Your stake was dropped on the ground");
					} else {
						winner.getInventory().addItem(item2);
					}
				}
			}
		}
	...
	private void beginBattle(boolean started) {
		...
		if (target.getLastDuelRules().getStake().getSize() < 1) {
			target.didStake = false;
		}
		player.checkBoosts();
		player.stopAll();
		player.lock(10); // fixes mass click steps
		player.reset();
		isDueling = true;
	...
	public void process() {
		player.checkBoosts();
		if (target == null || target.hasFinished() || player == null
				|| player.hasFinished()) {
			emergencyEnd();
		}
	}
	...
	public boolean processObjectClick1(WorldObject object) {
		if (target.getHitpoints() < 1 || target.isDead() || player.isDead()
				|| player.getHitpoints() < 1)
			return false;
		if (!ifFriendly) {
			player.getPackets().sendGameMessage("You cannot forfeit during a staked duel.");
		return ifFriendly;	
		}
		player.getDialogueManager().startDialogue("ForfeitDialouge", target,
				player);
		return true;
	}
	...
	public boolean sendDeath() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			@Override
			public void run() {
				if (target.getHitpoints() == 0 || target.isDead()) {
					tie = true;
				}
				player.stopAll();
				if (loop == 0) {
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		synchronized (this) {
			synchronized (target.getControlerManager().getControler()) {
				DuelRules rules = player.getLastDuelRules();
				if (interfaceId == 749 && componentId == 4 && rules.getRule(5)
						&& isDueling) {
					player.getPackets().sendGameMessage(
				...
				case 631:
					...
					case 58: // no magic
						...
						return false;
					case 59: // fun wep
						player.out("Fun weapons only has been disabled");
						return false;
					case 60: // no forfiet
						rules.setRules(7);
						return false;
					...
					case 46:
						if (target.getLastDuelRules().getStake().getSize() < 1
								&& player.getLastDuelRules().getStake()
										.getSize() < 1) {
							player.out("You both need to have a stake.");
							target.out("You both need to have a steak");
							return true;
						}
						accept(true);
						return false;
